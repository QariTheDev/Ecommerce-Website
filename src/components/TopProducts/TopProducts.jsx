    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                let fiveStarProducts = data.filter(product => product.rating.rate > 4);
                let shuffledData = fiveStarProducts.sort(() => 0.5 - Math.random());

                const selectedProducts = shuffledData.slice(0, 3);
            ...
            {loading ? (
                ...
                >
                    <div className="flex flex-col items-center justify-center space-y-4 py-10 px-4 sm:px-8 lg:px-12 xl:px-24">
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">Top Rated Products</h1>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10 xl:gap-12">
                            {products.map((product, index) => (
                                <div key={index} className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105">
